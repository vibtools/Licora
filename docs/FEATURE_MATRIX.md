...
| Dashboard reload-free refresh | Implemented in v5.7.0 source baseline; v5.7.1 corrective candidate | Dedicated Dashboard controller uses authenticated 30-second AJAX polling, manual refresh, in-place KPI/chart/activity updates, overlap prevention, last-updated feedback and stale/auth states while preserving server-rendered fallback. |
