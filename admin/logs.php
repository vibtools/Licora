...
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Activity Logs · Licora</title>
    <link rel="icon" href="assets/brand/favicon/favicon.ico"><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.1/font/bootstrap-icons.css"><link rel="stylesheet" href="assets/css/admin-ui.css">
</head>
<body class="admin-ui">
<?php include 'includes/navbar.php'; ?>
<div class="container-fluid admin-shell">
    <div class="page-hero d-flex flex-column flex-xl-row justify-content-between align-items-xl-center gap-3">
        <div><h2><i class="bi bi-clock-history"></i> Activity Logs</h2></div>
    ...
    <div class="card">
        <div class="card-header"><h5 class="mb-0"><i class="bi bi-clock-history"></i> Activity Logs</h5></div>
        <div class="card-body border-bottom py-2">
            <form method="GET" class="ui-table-toolbar" data-no-spinner><div class="ui-table-toolbar-main">
                <select name="action" class="form-select"><option value="">All Actions</option><?php foreach ($actions as $action): ?><option value="<?php echo Security::escape($action['action']); ?>" <?php echo $filter_action == $action['action'] ? 'selected' : ''; ?>><?php echo Security::escape($action['action']); ?></option><?php endforeach; ?></select>
                <input type="search" class="form-control" name="license" placeholder="License Key" value="<?php echo Security::escape($filter_license); ?>">
                <input type="date" class="form-control" name="date" value="<?php echo Security::escape($filter_date); ?>">
                </div><div class="ui-table-toolbar-actions"><button type="submit" class="btn btn-primary"><i class="bi bi-funnel"></i> Apply</button><?php if ($filter_action || $filter_license || $filter_date): ?><a class="btn btn-outline-secondary" href="logs.php">Reset</a><?php endif; ?></div>
            </form>
        </div>
        <div class="card-body">
            <div class="table-responsive ui-scrollbar">
                <table class="table table-hover align-middle" id="logs-table" data-ui-paginate="true" data-ui-page-size="10">
                    ...
                    <tbody>
                    <?php if (empty($logs)): ?>
                        <tr><td colspan="5"><div class="empty-state"><div class="empty-icon"><i class="bi bi-journal-text"></i></div><h5>No logs found</h5></div></td></tr>
                    <?php else: ?>
            ...
            <?php if (!empty($logs)): ?><div class="ui-table-footer"><span><?php echo count($logs); ?> entries</span><nav aria-label="Logs pagination"><ul class="pagination mb-0" data-ui-pager-for="logs-table"></ul></nav></div><?php endif; ?>
        </div>
    </div>
</div>
