...
foreach(['includes/navbar.php','assets/css/admin-ui.css','assets/css/licora-updater.css','licora-update-log-modal','licora-update-confirm-modal','Copy Logs','Download Diagnostics','Pin to bottom'] as $m){uiu_ok(strpos($page,$m)!==false,'updater UI contract missing '.$m);}
...
uiu_ok(strpos($page,'data-confirm-close')!==false,'shared updater confirmation close contract missing');uiu_ok(strpos($page,'AI Diagnostics')===false,'demo-only AI Diagnostics must remain absent');
