...
require_once dirname(__DIR__).'/includes/v2/V2KeyManager.php';
require_once dirname(__DIR__).'/includes/v2/V2TokenService.php';
require_once dirname(__DIR__).'/includes/v2/V2Repository.php';
require_once dirname(__DIR__).'/includes/v2/V2Provisioner.php';
function ok($v,$m){if(!$v){fwrite(STDERR,"FAIL: {$m}\n");exit(1);}}
$tmp=sys_get_temp_dir().'/licora-v2-db-'.bin2hex(random_bytes(6)); mkdir($tmp,0700,true);
$private=$tmp.'/private.pem'; $public=$tmp.'/public.pem';
register_shutdown_function(static function() use ($private,$public,$tmp) { @unlink($private); @unlink($public); @rmdir($tmp); });
$keys=new V2KeyManager($private,$public,'db-test-key');
$provisioner=new V2Provisioner($db,$keys,dirname(__DIR__).'/migration-v5.2.0-api-v2.sql');
$provisioned=$provisioner->provision(false);
ok(!empty($provisioned['ready']) && !empty($provisioned['signing_keys_generated']),'shared v2 provisioner');
$provisionedAgain=$provisioner->provision(false);
ok(!empty($provisionedAgain['ready']) && empty($provisionedAgain['signing_keys_generated']),'shared v2 provisioner is idempotent and retains existing signing keys');
...
$old=$repo->createRefreshToken((int)$r['credential']['id'],86400);
$rateCtx=$repo->refreshRateLimitContext($old['token']); ok(is_array($rateCtx) && $rateCtx['app_id']==='vibrapilot','refresh rate-limit identity context'); ok(!$db->inTransaction(),'refresh rate-limit context is read outside refresh transaction');
$ctx=$repo->refreshContextForUpdate($old['token']); $new=$repo->completeRefresh($ctx); ok($new['token']!==$old['token'],'refresh rotation');
