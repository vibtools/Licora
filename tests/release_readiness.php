...
$releaseNotes = $read('RELEASE_NOTES_v5.2.0.md');
$changelog = $read('CHANGELOG.md');
$configuration = $read('docs/CONFIGURATION.md');
$releaseGuide = $read('docs/RELEASE.md');
$packager = $read('scripts/package-release.sh');

$versionDefinition = "if (!defined('APP_VERSION')) define('APP_VERSION', env_value('APP_VERSION', '5.2.0'));";
...
$assert(strpos($installation, "'APP_VERSION' => '5.2.0'") !== false, 'generated installer configuration targets v5.2.0');
$assert(strpos($installer, 'Professional installation wizard for Licora v5.2.0') !== false, 'installer branding targets v5.2.0');
...
$assert(strpos($releaseNotes, 'Licora v5.2.0') !== false, 'release notes identify v5.2.0');
$assert(strpos($changelog, '## [5.2.0] - 2026-08-08') !== false, 'changelog contains the publication date');
$assert(strpos($configuration, '`APP_VERSION` | `APP_VERSION` | `5.2.0`') !== false, 'configuration reference matches runtime version');
$assert(strpos($releaseGuide, 'scripts/package-release.sh v5.2.0 v5.2.0') !== false, 'release guide uses the v5.2.0 packager command');
...
foreach ([
    ...
    $content = $read($path);
    $assert(strpos($content, '5.2.1') === false, 'v5.2.0 release file does not contain future version marker: ' . $path);
}
