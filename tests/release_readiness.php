...
$releaseNotes = $read('RELEASE_NOTES_v5.1.0.md');
$changelog = $read('CHANGELOG.md');
$configuration = $read('docs/CONFIGURATION.md');
$releaseGuide = $read('docs/RELEASE.md');
$packager = $read('scripts/package-release.sh');

$versionDefinition = "if (!defined('APP_VERSION')) define('APP_VERSION', env_value('APP_VERSION', '5.1.0'));";
...
$assert(strpos($installation, "'APP_VERSION' => '5.1.0'") !== false, 'generated installer configuration targets v5.1.0');
$assert(strpos($installer, 'Professional installation wizard for Licora v5.1.0') !== false, 'installer branding targets v5.1.0');
...
$assert(strpos($releaseNotes, 'Licora v5.1.0') !== false, 'release notes identify v5.1.0');
$assert(strpos($changelog, '## [5.1.0] - 2026-08-06') !== false, 'changelog contains the publication date');
$assert(strpos($configuration, '`APP_VERSION` | `APP_VERSION` | `5.1.0`') !== false, 'configuration reference matches runtime version');
$assert(strpos($releaseGuide, 'scripts/package-release.sh v5.1.0 v5.1.0') !== false, 'release guide uses the v5.1.0 packager command');
...
foreach ([
    ...
    $content = $read($path);
    $assert(strpos($content, '5.1.1') === false, 'v5.1.0 release file does not contain future version marker: ' . $path);
}
