...
final class ManifestVerifier
{
    ...
    public function validate(array $m,string $expectedVersion,string $currentVersion): void
    {
        ...
        if(($m['channel']??'')!=='stable'){throw new UpdateException('UPDATE_MANIFEST_INVALID','Only the signed stable update channel is supported.',400);} $minimumUpdater=(string)($m['minimum_updater']??'');$minimumPhp=(string)($m['minimum_php']??'');if(!preg_match('/^\d+\.\d+\.\d+$/',$minimumUpdater)||!preg_match('/^\d+\.\d+(?:\.\d+)?$/',$minimumPhp)){throw new UpdateException('UPDATE_MANIFEST_INVALID','Update manifest compatibility versions are invalid.',400);}
        if(!preg_match('/^\d+\.\d+\.\d+$/',$currentVersion)){throw new UpdateException('UPDATE_SOURCE_VERSION_INVALID','Installed Licora version is not a supported semantic version.',409);} $upgradeFrom=$m['upgrade_from']??null;if(!is_array($upgradeFrom)||$upgradeFrom===[]){throw new UpdateException('UPDATE_MANIFEST_INVALID','Update manifest is missing its supported source-version contract.',400);} $seenSources=[];foreach($upgradeFrom as $sourceVersion){$sourceVersion=(string)$sourceVersion;if(!preg_match('/^\d+\.\d+\.\d+$/',$sourceVersion)||isset($seenSources[$sourceVersion])){throw new UpdateException('UPDATE_MANIFEST_INVALID','Update manifest contains an invalid or duplicate source version.',400);}$seenSources[$sourceVersion]=true;}if(!isset($seenSources[$currentVersion])){throw new UpdateException('UPDATE_SOURCE_VERSION_UNSUPPORTED','This release does not support a direct update from Licora '.$currentVersion.'. Install a supported intermediate release first.',409);}
