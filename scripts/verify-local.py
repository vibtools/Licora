#!/usr/bin/env python3
"""Local source verifier for Licora v5.5.1.
...
if f"env_value('APP_VERSION', '{VERSION}')" not in config:
    fail("runtime APP_VERSION is not 5.5.1")
for rel in ["config.sample.php", "install.php", "includes/installation.php", "RELEASE_NOTES_v5.5.1.md", "CHANGELOG.md", "REPOSITORY_METADATA.md"]:
    if VERSION not in read(rel):
        fail(f"5.5.1 release marker missing from {rel}")
...
for marker in ['\"protocol_version\": 1', '\"version\": \"5.5.1\"', '\"minimum_updater\": \"5.3.0\"', '\"upgrade_from\"']:
...
if 'migration-v5.3.0-updater.sql' in release_spec:
    fail('v5.5.1 UI hotfix release spec must not replay the v5.3.0 updater migration')
import json as _json
_release_spec_data = _json.loads(release_spec)
if _release_spec_data.get("upgrade_from") != ["5.5.0"]:
    fail("v5.5.1 release spec must accept exactly the v5.5.0 official baseline")
if _release_spec_data.get("migrations") != []:
    fail("v5.5.1 UI hotfix must not declare a database migration")
...
print("Licora v5.5.1 local verification passed.")
